...
- **题解区（洛谷式）**：独立题解页 `/oj/solutions/:id`；OJ 详情右侧常驻题解卡片，点击进入题解区；页内右上角“发题解”展开编辑器；仅 AC 过该题的用户可发布；题解详情复用讨论详情页并显示“题解”徽标；普通讨论列表自动排除题解。
- **富文本编辑器**：已重做——更全工具栏（标题/引用/删除线/撤销重做/清除格式）、预览模式、更好用的编辑区。
- **每日一题 + AC 连击**（留存机制，已上线）：
