...
export default function EditProblemPage() {
  ...
  const [status, setStatus] = useState<'draft' | 'pending_review' | 'published' | 'hidden'>('draft')
    ...
    try {
      const { response, data } = await fetchJson<{
        problem: {
          ...
          editorialStatus?: ProblemEditorialStatus
      ...
      setDifficulty(getDifficultyMeta(problem.difficulty).key)
      setTags(problem.tags || [])
      setTopicTags(problem.topicTags || [])
      setTechniqueTags(problem.techniqueTags || [])
      setEstimatedMinutes(problem.estimatedMinutes ? String(problem.estimatedMinutes) : '')
      setRecommendedFor(problem.recommendedFor || '')
      setQualityStatus(problem.qualityStatus || 'unchecked')
      setEditorialStatus(problem.editorialStatus || 'none')
      setRevisionSummary(problem.revisionSummary || '')
      ...
      setStatus(problem.status === 'published' || problem.status === 'hidden' || problem.status === 'pending_review' ? problem.status : 'draft')
        ...
        <div className="problem-editor-shell">
          <main className="problem-editor-main">
            ...
            <Panel className="problem-editor-card">
              ...
              <div className="form-row problem-metadata-row">
                <div className="form-section">
                  <label className="form-label" htmlFor="edit-problem-estimated-minutes">预计用时（分钟）</label>
                  <input
                    id="edit-problem-estimated-minutes"
                    type="number"
                    className="auth-input"
                    min={1}
                    max={600}
                ...
                <div className="form-section">
                  <label className="form-label" htmlFor="edit-problem-recommended-for">适合人群</label>
                  <input
                    id="edit-problem-recommended-for"
                    type="text"
                    className="auth-input"
                    maxLength={120}
              ...
              {currentUser.isAdmin && (
                <div className="form-row problem-metadata-row">
                  <div className="form-section">
                    <label className="form-label" htmlFor="edit-problem-quality-status">内容质量</label>
                    <select id="edit-problem-quality-status" className="auth-input" value={qualityStatus} onChange={(event) => setQualityStatus(event.target.value as ProblemQualityStatus)}>
                  ...
                  <div className="form-section">
                    <label className="form-label" htmlFor="edit-problem-editorial-status">题解状态</label>
                    <select id="edit-problem-editorial-status" className="auth-input" value={editorialStatus} onChange={(event) => setEditorialStatus(event.target.value as ProblemEditorialStatus)}>
              ...
              <div className="form-section">
                <label className="form-label" htmlFor="edit-problem-revision-summary">本次修改说明</label>
                <input
                  id="edit-problem-revision-summary"
                  type="text"
                  className="auth-input"
                  maxLength={500}
