...
describe('SS-AUTH-003 production deployment contract', () => {
  ...
  it('schedules the installed root-owned backup copy through a sandboxed UTC systemd timer', async () => {
    ...
    expect(service).toContain('ConditionPathExists=/usr/local/sbin/starstack-backup')
    expect(service).not.toContain('ConditionPathIsRegular')
    expect(service).toContain('ExecCondition=/usr/bin/test -f /usr/local/sbin/starstack-backup')
    expect(service).toContain('ExecCondition=/usr/bin/test ! -L /usr/local/sbin/starstack-backup')
  ...
  it('ships a read-only preflight and separate backup tooling', async () => {
    ...
    expect(preflight).toContain("'/etc/systemd/system/starstack-api.service'")
    expect(preflight).toContain("'/proc/sys/kernel/dmesg_restrict'")
    expect(preflight).toContain('assertJudgeSystemdUnit(')
    expect(preflight).toContain('assertJudgeKernelPrerequisites(')
