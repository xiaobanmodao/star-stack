...
- OIDC 默认关闭；启用时 issuer 在开发环境固定为 `http://auth.localhost:5174`、生产固定为 `https://auth.xingzhan.cc`。Hydra Public/Admin origin 仅接受完整私网 IP、`localhost` 或单标签容器服务名，拒绝伪装成私网前缀的公网 DNS。本地精确 `hydra_test` DSN 只允许 canonical mode-`0600` 凭据，协议门禁会将凭据与 Hydra/StarStack fixture 数据作为一个单元轮换重建，并在实际 `run-local` 重启后验证 Discovery/JWKS 与稳定 `kid`。尚未接入生产 DNS/Nginx/PM2，真实 Hydra v26.2.0 + PostgreSQL 16.15 的启动、协议测试、备份与停止线见 `infra/identity/README.md`。
