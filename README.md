...
- OIDC 默认关闭；启用时 issuer 在开发环境固定为 `http://auth.localhost:5174`、生产固定为 `https://auth.xingzhan.cc`。Hydra Public/Admin origin 仅接受完整私网 IP、`localhost` 或单标签容器服务名，拒绝伪装成私网前缀的公网 DNS。尚未接入生产 DNS/Nginx/PM2，真实 Hydra v26.2.0 + PostgreSQL 16.15 的启动、协议测试、备份与停止线见 `infra/identity/README.md`。
